    ...
    >
      <div className="flex-shrink-0 w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-2xl">
        {emoji ? (
          ...
          <LucideIconComponent className="w-6 h-6" style={{ color: accentColor }} />
        ) : (
          <Link2 className="w-6 h-6" style={{ color: accentColor }} />
        )}
      </div>
      <div className="flex-1 min-w-0">
        <p className="font-semibold truncate" style={{ color: accentColor }}>{title}</p>
        {subtitle && (
          <p className="text-sm truncate" style={{ color: textColor }}>{subtitle}</p>
        )}
      </div>
    </a>
  );
